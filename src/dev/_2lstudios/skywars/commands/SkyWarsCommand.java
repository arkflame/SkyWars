...
public class SkyWarsCommand implements CommandExecutor {
  ...
  public SkyWarsCommand(Server server, ArenaManager arenaManager, GamePlayerManager playerManager) {
    this.server = server;
    ...
    if (sender instanceof Player) {
      ...
      if (args.length == 0) {
        player.sendMessage(ChatColor.GREEN + "Comandos de SkyWars:"
            .concat(System.lineSeparator() + ChatColor.YELLOW + "/" + label + " join [mapa]" + ChatColor.GRAY + " - "
                + ChatColor.AQUA + "Unete a una partida al azar!")
            .concat(System.lineSeparator() + ChatColor.YELLOW + "/" + label + " spec <mapa>" + ChatColor.GRAY + " - "
                + ChatColor.AQUA + "Espectea un mapa que especifiques!")
            .concat(System.lineSeparator() + ChatColor.YELLOW + "/" + label + " leave" + ChatColor.GRAY + " - "
                + ChatColor.AQUA + "Sal del mapa que estas jugando!")
            .concat(System.lineSeparator() + ChatColor.YELLOW + "/" + label + " list" + ChatColor.GRAY + " - "
                + ChatColor.AQUA + "Muestra una lista de mapas!"));
        ...
        if (args[0].equalsIgnoreCase("create") && args.length == 2) {
          ...
          if (this.arenaManager.getArena(arena) != null
              && this.arenaManager.getArena(arena).getState() == GameState.EDITING) {
